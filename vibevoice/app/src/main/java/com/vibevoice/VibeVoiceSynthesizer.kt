 ...
 * precomputed
 * prompt KV cache (voice_*.bin). No FFT anywhere. Graphs load from filesDir (push via
 * scripts/install_to_device.sh); the fp16 token-embedding table is mmapped.
 */
class VibeVoiceSynthesizer(private val context: Context) : Closeable {
    ...
    companion object {
        ...
        const val BASE_L = 4; const val BASE_PMAX = 128
        const val TTS_L = 20; const val TTS_PMAX = 384
        const val TEXT_WIN = 5; const val SPEECH_WIN = 6
        const val CFG = 1.3f
    ...
    // directly (no tmp double-copy) — see scripts/install_to_device.sh.
    ...
    private fun path(name: String): File {
        val f = File(modelDir, name)
        check(f.exists()) {
            "Asset not found: $name. Push it first:\n  scripts/install_to_device.sh"
        }
        return f
    }
    ...
    // glue.f32 offsets (float indices), see export_vibevoice_assets.py
    private val connFc1W = 0; private val connFc1B = 57344; private val connNormW = 58240
    private val connFc2W = 59136; private val connFc2B = 861952; private val typeEmb = 862848
    private val eosFc1W = 864640; private val eosFc1B = 1667456; private val eosFc2W = 1668352
        ...
        fun seed(c: Cache, seedLen: Int) {
            fun read(dst: FloatArray) {
                for (gi in 0 until c.g) for (s in 0 until seedLen) {
                    ...
                    for (d in 0 until HD) dst[base + d] = Half.toFloat(bb.short)
                }
            }
            read(c.pk); read(c.pv); c.pos = seedLen
        }
        seed(baseCache, 74)
        seed(ttsCache, 251)
        seed(negCache, 1)
        for (i in 0 until H) ttsInitHidden[i] = bb.float
        for (i in 0 until H) negInitHidden[i] = bb.float
    }

    // ---- small host math --------------------------------------------------
    private fun embRow(id: Int): FloatArray {
        val out = FloatArray(H); var b = id.toLong() * H * 2L
        for (j in 0 until H) { out[j] = Half.toFloat(embMap.getShort(b.toInt())); b += 2 }
        return out
    }
    ...
    private fun linear(x: FloatArray, w: Int, b: Int, outDim: Int, inDim: Int): FloatArray {
        ...
        for (o in 0 until outDim) {
            var acc = glue[b + o]; val row = w + o * inDim
            for (i in 0 until inDim) acc += glue[row + i] * x[i]
            out[o] = acc
        }
        return out
    }
    ...
    private fun connector(latent: FloatArray): FloatArray {
        ...
        var ms = 0f; for (v in h1) ms += v * v; ms /= H
        val r = (1.0 / sqrt(ms + 1e-6)).toFloat()                    // RMSNorm (eps 1e-6)
        for (o in 0 until H) h1[o] = h1[o] * r * glue[connNormW + o]
    ...
    private fun eosProb(hidden: FloatArray): Float {
        ...
        for (o in 0 until H) if (e1[o] < 0f) e1[o] = 0f              // relu
        var acc = glue[eosFc2B]; val row = eosFc2W
        for (i in 0 until H) acc += glue[row + i] * e1[i]           // fc2: 896 -> 1
        return (1.0 / (1.0 + exp(-acc.toDouble()))).toFloat()
    }

    private fun addType(hidden: FloatArray, type: Int): FloatArray {
        val out = FloatArray(H); val base = typeEmb + type * H
        for (o in 0 until H) out[o] = hidden[o] + glue[base + o]
        return out
    }

    private fun rope(pos: Int): Pair<FloatArray, FloatArray> {
        val cos = FloatArray(HD); val sin = FloatArray(HD)
        for (j in 0 until HD / 2) {
            val ang = pos * (1.0 / Math.pow(THETA, 2.0 * j / HD))
            val c = kotlin.math.cos(ang).toFloat(); val s = kotlin.math.sin(ang).toFloat()
            cos[j] = c; cos[j + HD / 2] = c; sin[j] = s; sin[j + HD / 2] = s
        }
        return cos to sin
    }
    ...
    private fun lmStep(
        ...
        inB[0].writeFloat(x); inB[1].writeFloat(cos); inB[2].writeFloat(sin)
        inB[3].writeFloat(cache.mask()); inB[4].writeFloat(cache.pk); inB[5].writeFloat(cache.pv)
        model.run(inB, outB)
    ...
    private fun headV(noisy: FloatArray, tEmb: FloatArray, cond: FloatArray): FloatArray {
        headIn[0].writeFloat(noisy); headIn[1].writeFloat(tEmb); headIn[2].writeFloat(cond)
    ...
    private fun sampleLatent(cond: FloatArray, negCond: FloatArray): FloatArray {
        ...
        for (i in 0 until TIMESTEPS.size) {
            ...
            val lamT = ln(aT) - ln(sgT); val lamS0 = ln(aS0) - ln(sgS0); val h = lamT - lamS0
            val c1 = aT * (exp(-h) - 1.0)
    ...
    fun synthesize(text: String, maxSpeechTokens: Int = TTS_PMAX - 260): Result {
        ...
        var winIdx = 0; var finished = false
        while (!finished) {
    ...
    private fun decode(latents: List<FloatArray>): FloatArray {
        ...
        for (f in 0 until n) for (c in 0 until VAE) x[c * DECODE_FRAMES + f] = latents[f][c]
    ...
    override fun close() {
        ...
        baseLm.close(); ttsLm.close(); head.close(); decoder.close(); embChannel.close()
    }

    private fun readF32(f: File): FloatArray {
        val b = f.readBytes(); val bb = ByteBuffer.wrap(b).order(ByteOrder.LITTLE_ENDIAN)
