...
import _stub_propack, sys, os, types, collections
...
def build(swap):
    ...
    m = BaseOCRV20(cfg); m.net.load_state_dict(load_file(W_REC), strict=False); m.net.eval()
    if swap:
...
def main():
    ...
    orig = build(False); reauth = build(True)
    x = torch.randn(1, 3, Hh, Ww)
    with torch.no_grad():
        a = RecWrap(orig)(x); b = RecWrap(reauth)(x)
    ...
    if stage == "all":
        to_fp16(fp32, os.path.join(HERE, "ppocr_rec_fp16.tflite")); opcheck(os.path.join(HERE, "ppocr_rec_fp16.tflite"), "rec_fp16")
