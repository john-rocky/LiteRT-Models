 ...
 *
 * Preview and analysis share a 4:3 aspect ratio so the FILL_CENTER overlay
 * (DetectionCanvasView) lines up with the preview without a ViewPort.
 *
 * The [onFrame] callback runs on the inference thread. Post UI updates with
 * `view.post { ... }`. To gate inference (e.g. while loading models), set
 * [enabled] to false.
 */
