 ...
 * Validated vs transformers Mimi: encode codes match 100% (torch latent) / decode bit-exact.
 */
class MimiRvq(binPath: String) {
    ...
    fun encode(emb: FloatArray, T: Int): IntArray {
        ...
        // semantic: 1 codebook, no residual loop needed
        for (t in 0 until T) {
            project(semWin, emb, t, T, proj)
            codes[t] = nearest(proj, semCB[0], semCBnorm[0])
        }
        // acoustic: 31 codebooks, residual loop in DIM space
