 ...
 * run on CPU, and the split RVQ runs on CPU in [MimiRvq].
 *
 ...
 *
 * Why the transformers are on CPU: the decoder transformer's residual stream reaches |x|=27, and the
 * Mali GPU delegate's internal fp16 compute loses precision there (device audio ~12 dB SNR), while
 * the SEANet convs are fp16-exact on GPU (~48 dB). Standalone == fused on device, so this is fp16
 * PRECISION, not a fusion collapse — the C33 Matcha bug does NOT generalize to Mimi's transformer.
 * The transformers are tiny (8 layers x 512, seq~50), so CPU is trivial. See docs/LITERT_CONVERSION_GUIDE.md.
 *
 * All graphs + mimi_rvq.bin are loaded from filesDir (push via scripts/install_to_device.sh).
...
class MimiCodec(private val context: Context) : Closeable {
    ...
    private fun load(name: String, acc: Accelerator): CompiledModel {
        ...
        check(f.exists()) { "Model not found: $name. Push first:\n  scripts/install_to_device.sh" }
