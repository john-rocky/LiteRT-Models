 ...
 * that runs on the LiteRT CompiledModel GPU. This mirrors the Whisper/Kokoro on-device pattern.
 *
...
class MelSpectrogram(context: Context) {
    ...
    init {
        ...
        require(melBasis.size == N_MELS * N_FREQS) { "mel_basis.bin has ${melBasis.size} floats, expected ${N_MELS * N_FREQS}" }
    }
    ...
    /**
     * Compute the log-mel spectrogram from a CLIP_SAMPLES-long 32 kHz mono clip.
     * Pads/truncates to CLIP_SAMPLES.
