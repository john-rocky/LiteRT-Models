 ...
 * on the host (FFT is GPU-banned). Pipeline, verified bit-faithful enough to reproduce the reference
 * transcript: preemphasis(0.97) -> center zero-pad(n_fft/2) -> frame -> hann(400) padded to 512 ->
 * |rFFT512|^2 (mag_power=2) -> mel filterbank (slaney) -> log(x + 2^-24) -> per-feature normalize (over
 * time, std with Bessel n-1 correction + 1e-5).
 *
...
class MelSpectrogram(private val fb: FloatArray) {
    ...
    private val window = FloatArray(N_FFT).also { w ->
        val off = (N_FFT - WIN) / 2
    ...
    fun compute(audio: FloatArray): Result {
        ...
        val re = FloatArray(N_FFT); val im = FloatArray(N_FFT)
        ...
        for (m in 0 until N_MEL) {
            ...
            mean /= nfr
            var v = 0f
            for (t in 0 until nfr) { val d = logmel[base + t] - mean; v += d * d }
    ...
    private class Fft(private val n: Int) {
        ...
        init {
            var log = 0; while ((1 shl log) < n) log++
            for (i in 0 until n) {
                var x = i; var r = 0
            ...
            for (k in 0 until n / 2) {
                ...
                cosT[k] = cos(a).toFloat(); sinT[k] = Math.sin(a).toFloat()
            }
        }
        ...
        fun transform(real: FloatArray, re: FloatArray, im: FloatArray) {
            for (i in 0 until n) { re[rev[i]] = real[i]; im[rev[i]] = 0f }
            var len = 2
            while (len <= n) {
