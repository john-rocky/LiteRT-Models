...
/**
 * Gates all 80 matched-fp16 fixture paths plus 60 forced-s256 paths against Python and the official
 * fp32 entity oracle.
 */
class GliformerFp16DecoderParityTest {
  @Test
  fun matchedFp16CpuLogitsMatchPythonAndOracle() {
    val fixtures = ExternalTestData.resolve()
    val originalCorpus = JSONObject(File(fixtures, "corpus.json").readText()).getJSONArray("rows")
    val originalById =
      (0 until originalCorpus.length()).associate {
        originalCorpus.getJSONObject(it).let { row -> row.getString("id") to row }
      }
    val manifest = JSONObject(File(fixtures, "references_fp16.json").readText())
    assertEquals("Selected reference rows", 80, manifest.getJSONArray("rows").length())
    assertEquals("Forced s256 rows", 60, manifest.getJSONArray("forced_rows").length())
    val entries =
      listOf("rows", "forced_rows").flatMap { name ->
        val values = manifest.getJSONArray(name)
        (0 until values.length()).map { values.getJSONObject(it) }
      }
    val logitRows = JSONArray(entries)
    val corpus =
      JSONArray(
        entries.map { entry ->
          JSONObject(originalById.getValue(entry.getString("id")).toString())
            .put("window", entry.getInt("window"))
        }
      )
    val logitById =
      (0 until logitRows.length()).associate { index ->
        logitRows.getJSONObject(index).let { "${it.getString("id")}_s${it.getInt("window")}" to it }
      }
    assertEquals("Original fixture count", 140, corpus.length())
    assertEquals("Logit manifest must contain every fixture exactly once", 140, logitRows.length())
    assertEquals("Logit IDs must be unique", 140, logitById.size)
    ...
    for (index in 0 until corpus.length()) {
      ...
      val entry = requireNotNull(logitById["${id}_s$window"]) { "Missing logits for $id" }
      ...
      val bytes = File(fixtures, entry.getString("asset_file")).readBytes()
    ...
    val report =
      JSONObject()
        ...
        .put("rows", rows)
    val reportFile = ExternalTestData.reportFile("decoder_parity_fp16.json")
    reportFile.writeText(report.toString(2) + "\n")
    println(
      "Matched fp16 decoder: Python $pythonIdentical/140, oracle $oracleIdentical/140, order $pythonOrderIdentical/140; max score error Python=$maxPythonDifference, oracle=$maxOracleDifference"
    )
