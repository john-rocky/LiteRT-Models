...
def _gs(input, grid, mode="bilinear", padding_mode="zeros", align_corners=None):   # GATHER/CAST-free bilinear (tent weights + BMM)
...
F.grid_sample=_gs; _T._bilinear_grid_sample=lambda input,grid,padding_mode="zeros",align_corners=False: _gs(input,grid,padding_mode=padding_mode,align_corners=align_corners)
...
def _core_patched(value, value_spatial_shapes, sampling_locations, attention_weights, value_spatial_shapes_hw=None):
    ...
    grids=2*sampling_locations-1; svl=[]
...
import math as _math, rfdetr.models.transformer as _TR
_DIMT={}
def _gen_sine(pos_tensor, dim=128):
    scale=2*_math.pi
    if dim not in _DIMT:
        dt=torch.arange(dim,dtype=torch.float32); _DIMT[dim]=(10000.0**(2*(dt//2)/dim)).detach()
    dim_t=_DIMT[dim]
    def il(emb):
...
def build():
    ...
    emb=bb.embeddings; C=emb.cls_token.shape[-1]; N=(R//16)**2+1
    ...
    class Wrap(nn.Module):
        def __init__(s): super().__init__(); s.net=net
        def forward(s,x):
            o=s.net.forward_export(x)
...
if __name__=="__main__":
    ...
    try:
        litert_torch.convert(g,(x,)).export(fp32); it=opcheck(fp32,"rffull")
        ...
        for oi,od in enumerate(it.get_output_details()):
            o=it.get_tensor(od["index"]); r=ref[oi].numpy() if isinstance(ref,(list,tuple)) else ref.numpy()
            if o.size==r.size: print(f"  out{oi} corr {np.corrcoef(o.ravel(),r.ravel())[0,1]:.5f}")
    except Exception as e: import traceback; traceback.print_exc(); print("CONVERT FAIL:",repr(e)[:400])
