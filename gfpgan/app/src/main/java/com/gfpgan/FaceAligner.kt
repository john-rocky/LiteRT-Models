...
object FaceAligner {
    ...
    fun align(src: Bitmap, landmarks: FloatArray): Bitmap {
        ...
        Canvas(out).apply {
            drawColor(android.graphics.Color.rgb(135, 133, 132))   // facexlib neutral fill for out-of-image border
     ...
     * Returns the 2x3 affine [a, -b, tx, b, a, ty]. Solves the 4-param normal equations
     * (linear in a,b,tx,ty) — no SVD needed.
     */
    private fun similarity(src: FloatArray, dst: FloatArray): FloatArray {
        ...
        for (i in 0 until n) {
            ...
            val X = dst[2 * i].toDouble(); val Y = dst[2 * i + 1].toDouble()
        ...
        // Normal matrix A^T A (symmetric, from rows [x,-y,1,0] and [y,x,0,1]):
        val a = arrayOf(
