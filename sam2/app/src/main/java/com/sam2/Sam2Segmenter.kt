...
/**
 * SAM2 (Segment Anything 2, Hiera-Tiny) promptable segmentation on LiteRT CompiledModel GPU.
 *
 * Split (identical image-path shapes to EdgeTAM, so this reuses the same flat I/O layout):
 * image ENCODER (Hiera, GPU, once per image) -> embeddings + FPN features; prompt encoder
 * (Kotlin, per tap) -> sparse embedding; mask DECODER (GPU, per tap) -> 3 multimask logits.
 * Verified corr 1.0 vs the full PyTorch SAM2 model on the desktop interpreter.
 */
class Sam2Segmenter(context: Context) : AutoCloseable {
    ...
    companion object {
        ...
        private const val F0 = 32 * 256 * 256     // high-res feature s0
        private const val F1 = 64 * 128 * 128     // high-res feature s1
    ...
    init {
        // Both graphs run on the ML Drift GPU. The Hiera encoder is made GPU-clean at conversion
        // time by three numerically-identical rewrites (see scripts/convert_sam2.py): the windowed
        // positional embedding is baked to a constant (removes the bicubic GATHER_ND), and window
        // partition + multi-scale attention are re-expressed with only <=4-D tensors (ML Drift
        // rejects >4-D). The mask decoder converts unchanged.
        encoder = load(context, "sam2_encoder.tflite", Accelerator.GPU)
        decoder = load(context, "sam2_decoder.tflite", Accelerator.GPU)
        val buf = context.assets.open("sam2_prompt.bin").use { it.readBytes() }
