...
MID = "facebook/wav2vec2-base-960h"
SR, DUR = 16000, float(os.environ.get("W2V_SEC","5"))
torch.manual_seed(0)
...
class CTCWrap(nn.Module):
    """waveform -> per-frame CTC logits [1, T', vocab]; attention_mask=None (fixed-length, no padding)."""
...
def main():
    ...
    m2 = Wav2Vec2ForCTC.from_pretrained(MID).eval()
    swap(m2)
    fold_weight_norm(m2)
    g = CTCWrap(m2).eval()
    with torch.no_grad():
    ...
    print(f"re-authored vs orig torch: logits corr {np.corrcoef(ref.numpy().ravel(), ra.numpy().ravel())[0,1]:.6f} "
          f"max|d| {(ref-ra).abs().max():.2e}  shape {tuple(ra.shape)}")

    import litert_torch
    fp32 = os.path.join(HERE, "w2v2_ctc.tflite")
    ...
    if stage == "all" and clean:
        f16 = to_fp16(fp32, os.path.join(HERE, "w2v2_ctc_fp16.tflite"))
