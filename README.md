...
| [Sopro v2 turbo](#sopro-v2-turbo-zero-shot-voice-cloning) | Text-to-speech (zero-shot voice cloning, EN / PT / FR / DE) | Mac CPU (device pending) | RTF 0.44 (M4 Max, contended) | [🤗 HF](https://huggingface.co/litert-community/sopro-v2-turbo) |
...
| [Laya English + typed-decisions](#laya-multilingual-typed-text-decisions) | English text classification / typed decisions with request-time questions (ModernBERT-large) | Galaxy S26 | 123 ms per question (GPU) | [🤗 HF](https://huggingface.co/litert-community/Laya-English-LiteRT) |
...
```bash
pip install ai-edge-litert==2.2.0 ai-edge-litert-sdk-qualcomm==2.2.0
```
...
- The SDK package downloads QAIRT at `pip install` time and **swallows a truncated
  download**, leaving a wheel with no `data/` directory. Verify it exists before compiling.
- A slim base image lacks `libc++1`, `libc++abi1` and `libatomic1`; the plugin `dlopen`
  reports them one at a time.
- `aot_compile()` does not forward arbitrary flags to the plugin. Call
...
- **Option state.** LiteRT's `Environment` is shared per process and the **first** model
  load fixes the dispatch options for every later one. A CPU model loaded first left the
  NPU without `BURST` and cost 6.2× on SSDLite. Measure one accelerator per process.
- **Execution order.** Pinning the NPU first fixed the NPU and made the GPU 29% slower —
  the confound moves to whatever you measure second.
...
**Sample app**: [gliner25/](gliner25/) — Kotlin host (word splitter + SentencePiece Unigram tokenizer, memory-mapped embedding lookup, float32 port of the upstream sparse decoder) in a Compose app: type text → highlighted entities with confidence and offsets, GPU / CPU selectable. On the Galaxy S26 (LiteRT 2.2.0, GPU FP32 precision, debug build) the app returns the official fp32 spans on 70/70 validation texts; at s128 the graph takes ~12 ms to readback and the Kotlin host decode ~10 ms (s512: ~92 ms / ~13 ms). The same module is mirrored in the HF repo under `android/`.
...
**On-device (Galaxy S26, LiteRT 2.2.0 — verified):** fully on the GPU delegate (1779/1779 ops, one partition) with `GpuOptions(precision = FP32)`; on 201 English/Japanese question rows the answers match the official fp32 `laya` 0.3.4 implementation (same argmax on 81/81 choice and score questions, max probability difference 0.0014 with fp16 weights); **GPU median 51 ms per question at 256 tokens** (CPU 163 ms), 60 ms end to end in the sample app. With the token table inside the graph, the fp16-weight file does not compile on ML Drift (the EMBEDDING_LOOKUP fed by a DEQUANTIZE is rejected: `Empty quantization params`); the fp32 in-graph table compiles (1.29 GB, ~7 s, the form shipped in [litert-community/laya-LiteRT](https://huggingface.co/litert-community/laya-LiteRT)), and fp16 FULLY_CONNECTED weights with DEQUANTIZE compile fully.
...
| Laya English s256 / s512, typed-decisions s256 / s512 | [HF: litert-community/Laya-English-LiteRT](https://huggingface.co/litert-community/Laya-English-LiteRT) | 740 / 741 MB fp16-weight (1,478 / 1,480 MB fp32) per checkpoint + 103 MB fp16 token table + 1.1 MB act head | inputs_embeds [1,N,1024] + attention_mask [1,N] + qtype_onehot [1,3] → token_logits [1,N] + pooled_cls [1,1024] → host marker gather + softmax; act head [1,1024]+[1,4] → [1,2] | GPU (s256 and s512 verified, both checkpoints) |

**English and typed-decisions checkpoints (ModernBERT-large, 421M):** the same graph cut, shipped as [litert-community/Laya-English-LiteRT](https://huggingface.co/litert-community/Laya-English-LiteRT) (root = English, `typed-decisions/` = the fine-tune, each with its own fp16 token table `[50368,1024]`, act head, tokenizer pair and config). On the Galaxy S26 (LiteRT 2.2.0, GPU FP32) every graph compiles as one partition (2223/2223 ops with fp16 FC weights, 2100/2100 fp32); on 140 English + 100 typed-decisions rows at 256 tokens and 209 + 175 at 512 the answers match the official fp32 implementation (same argmax on every choice/score question, max probability difference 0.0008 with fp16 weights, 6e-5 fp32); **GPU median 123 ms per question at 256 tokens (English), 126 ms (typed-decisions)**; the 512-token graphs measured 616–623 ms on an already warm phone (40–44 °C). The ModernBERT ByteLevel BPE tokenizer is not ported to Kotlin: the card's Kotlin block covers the table lookup and the graph call, and the Python host covers the whole pipeline.
