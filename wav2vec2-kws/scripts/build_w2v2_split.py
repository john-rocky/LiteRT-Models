...
"""
import _stub, os, numpy as np, torch, torch.nn as nn, torch.nn.functional as F
...
def main():
    ...
    import litert_torch
    fa = os.path.join(HERE, "w2v2_frontend.tflite"); fb = os.path.join(HERE, "w2v2_head.tflite")
    litert_torch.convert(fe, (x,)).export(fa)
    litert_torch.convert(hd, (feat,)).export(fb)
    ia, ca = B.opcheck(fa, "frontend"); ib, cb = B.opcheck(fb, "head")
    oa = B.tfl_run(ia, x.numpy()); ob = B.tfl_run(ib, feat.numpy())
