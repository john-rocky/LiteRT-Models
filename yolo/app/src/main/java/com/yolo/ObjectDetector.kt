 ...
 * YOLO11: bbox = (cx, cy, w, h) center format
 * YOLO26: bbox = (x1, y1, x2, y2) corner format
...
class ObjectDetector(context: Context, modelFileName: String = "yolo11n.tflite") : AutoCloseable {
    ...
    private val isXyxy: Boolean  // true for YOLO26 (x1,y1,x2,y2), false for YOLO11 (cx,cy,w,h)
    ...
    private fun postProcess(raw: FloatArray, origW: Int, origH: Int): List<Detection> {
        ...
        for (i in 0 until numDetections) {
            ...
            val r0 = raw[0 * numDetections + i]
            val r1 = raw[1 * numDetections + i]
            val r2 = raw[2 * numDetections + i]
            val r3 = raw[3 * numDetections + i]

            val xMin: Float; val yMin: Float; val xMax: Float; val yMax: Float
            if (isXyxy) {
                // YOLO26: x1, y1, x2, y2 (normalized 0-1)
                xMin = r0 * origW; yMin = r1 * origH
                xMax = r2 * origW; yMax = r3 * origH
            } else {
                // YOLO11: cx, cy, w, h (normalized 0-1)
                xMin = (r0 - r2 / 2f) * origW; yMin = (r1 - r3 / 2f) * origH
                xMax = (r0 + r2 / 2f) * origW; yMax = (r1 + r3 / 2f) * origH
            }
            ...
            candidates.add(Detection(
                ...
                score = maxScore,
                xMin = xMin.coerceIn(0f, origW.toFloat()),
                yMin = yMin.coerceIn(0f, origH.toFloat()),
                xMax = xMax.coerceIn(0f, origW.toFloat()),
                yMax = yMax.coerceIn(0f, origH.toFloat()),
            ))
        }
