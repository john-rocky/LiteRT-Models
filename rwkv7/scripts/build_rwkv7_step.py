...
Run:  python build_rwkv7_step.py [stage]
      stage in {parity, convert, fp16, assets, validate, all} (default: all)
"""
...
def stage_validate(sd, tok):
    """Runs the fp16 tflite through the CompiledModel API vs torch fp32."""
    from ai_edge_litert.compiled_model import CompiledModel

    ids = tok.encode(PARITY_PROMPT)
    with torch.no_grad():
        ref = gpt_mode_forward(sd, torch.tensor(ids))[0, -1].numpy()

    model = CompiledModel.from_file(FP16_PATH)
    inputs = model.create_input_buffers(0)
    outputs = model.create_output_buffers(0)
    emb = sd["emb.weight"].float().numpy()
    att = np.zeros((N_LAYER, N_EMBD), np.float32)
    ffn = np.zeros((N_LAYER, N_EMBD), np.float32)
    wkv = np.zeros((N_LAYER * N_HEAD, HEAD_DIM, HEAD_DIM), np.float32)
    logits = None
    for t in ids:
        inputs[0].write(np.ascontiguousarray(emb[t : t + 1]))
        inputs[1].write(att.ravel())
        inputs[2].write(ffn.ravel())
        inputs[3].write(wkv.ravel())
        model.run_by_index(0, inputs, outputs)
        logits = outputs[0].read(VOCAB_SIZE, np.float32)
        att = outputs[1].read(N_LAYER * N_EMBD, np.float32)
        ffn = outputs[2].read(N_LAYER * N_EMBD, np.float32)
        wkv = outputs[3].read(N_LAYER * N_HEAD * HEAD_DIM * HEAD_DIM, np.float32)
    corr = np.corrcoef(logits, ref)[0, 1]
    top = tok.decode([int(logits.argmax())])
    ref_top = tok.decode([int(ref.argmax())])
    print(f"validate: corr {corr:.7f}  top {top!r}  (fp32 top {ref_top!r})")
    assert corr > 0.999, "fp16 tflite diverges from fp32 torch"
