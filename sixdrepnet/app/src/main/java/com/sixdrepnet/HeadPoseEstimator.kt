 ...
 *
 * RepVGG-B1g2 (deploy) — pure CNN, fully GPU (zero patches). ~21 ms/frame.
 * Feed a face crop; this demo uses a centered square crop of the frame.
 */
class HeadPoseEstimator(context: Context, modelFileName: String = "6drepnet.tflite") : AutoCloseable {
    ...
    fun estimate(faceCrop: Bitmap): Pair<HeadPose, Long> {
        ...
        // Gram-Schmidt: x = norm(v0..2); z = norm(cross(x, v3..5)); y = cross(z, x). R columns [x|y|z].
        ...
        // rotation matrix R[row][col], col0=x, col1=y, col2=z
        val r00 = x[0]; val r10 = x[1]; val r20 = x[2]
        val r21 = y[2]; val r22 = z[2]
