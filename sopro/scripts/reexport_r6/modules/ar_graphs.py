...
class ARPrefillGraph(ARBase):
    input_names = ('prefix_embeddings', 'attention_bias', 'last_onehot')
    ...
    def forward(self, prefix_embeddings, attention_bias, last_onehot):
        ...
        for layer in self.ar.temporal.layers:
            ...
            keys.append(k)
            values.append(v)
        # Exactly one coefficient is 1, all other coefficients are 0. Keeping
        # both operands rank four avoids the delegate's rank-three BMM path.
        row = (last_onehot.reshape(1, 1, 1, n) @ x.reshape(1, 1, n, 512)).reshape(1, 1, 512)
