 ...
 *
 * Pure CNN (ResNet18 + row-classification head) — fully GPU with ZeroPadMaxPool for the
 * ResNet stem. The grid→lane decode (softmax + expectation per row) runs host-side.
 */
class LaneDetector(context: Context, modelFileName: String = "ufld.tflite") : AutoCloseable {

    companion object {
        private const val TAG = "UFLD"
        const val W = 800; const val H = 288
        const val GRIDING = 200          // + 1 "no lane"
        const val ROWS = 18; const val LANES = 4
        // CULane row anchors (in the 288-tall input), bottom-to-top after the flip below
    ...
    fun detect(bitmap: Bitmap): Pair<List<LanePoint>, Long> {
        ...
        // decode: for each lane & row, softmax over the 200 grid cells -> expectation column;
        // skip if the argmax over all 201 is the "no lane" class (index 200). Output row j
        // pairs directly with ROW_ANCHOR[j] (the reference's double row-flip cancels out).
        ...
        for (lane in 0 until LANES) {
            for (row in 0 until ROWS) {
                ...
                val col = wsum / sum                    // 1..200 expectation
                val xPix = (col - 1) * colStep          // 0..W
                val yPix = ROW_ANCHOR[row].toFloat()    // 0..H
                pts.add(LanePoint(lane, xPix / W, yPix / H))
            }
        }
