...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        bg.execute {
            try {
                ...
                Log.i(tag, "self-test 440 Hz -> ${p.note}${p.octave} ${"%.1f".format(p.hz)} Hz (${"%.1f".format(ms)} ms/frame)")
                runOnUiThread {
    ...
    private fun startListening() {
        ...
        bg.execute {
            ...
            val rec = try {
                AudioRecord(source, sr, AudioFormat.CHANNEL_IN_MONO, AudioFormat.ENCODING_PCM_16BIT, maxOf(minBuf, sr))
            } catch (e: Throwable) {
