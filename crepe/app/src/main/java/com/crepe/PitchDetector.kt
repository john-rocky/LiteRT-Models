 ...
 * the 360-bin → cents → Hz decode are host-side. The model is bundled in assets (44.5 MB fp16).
 */
class PitchDetector(context: Context) : Closeable {
    ...
    private val model = CompiledModel.create(
        context.assets, "crepe_full_fp16.tflite", CompiledModel.Options(Accelerator.GPU), null)
    ...
    fun detect(frame: FloatArray): Pitch {
        ...
        return toNote(hz, act[c])
    }

    private fun toNote(hz: Float, confidence: Float): Pitch {
        ...
        return Pitch(hz, confidence, name, octave, centsOff)
    }
