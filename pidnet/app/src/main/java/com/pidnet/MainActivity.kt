...
class MainActivity : ComponentActivity() {
    ...
    private fun loadModel() {
        backgroundExecutor.execute {
            try {
                ...
                statusText.post { statusText.text = "PIDNet-S GPU ready — Cityscapes 19 classes" }
    ...
    private fun runInference(bmp: Bitmap) {
        ...
        val (labels, ms) = seg.segment(bmp)
        overlayView.post { overlayView.setLabels(labels) }
        val fps = pipeline?.fps ?: 0
        statusText.post { statusText.text = "PIDNet-S GPU  |  $fps FPS  |  ${ms}ms/frame" }
    }
