...
the DiT sees, and writes the fixed tensors the device needs. The device then runs
...
def save(tensor, name):
    """Writes a tensor as little-endian float32 to OUT/<name>.bin."""
...
def save_int(array, name):
    """Writes an index array as little-endian int32 to OUT/<name>.bin."""
...
def tail_permutations(pipe, img_ids):
    ...
    arange probe recovers them exactly and the device can apply them as a gather.
    """
    ...
    pure_unpatch = np.array_equal(np.sort(unpatch_perm), np.arange(128 * 16 * 16))
...
def main():
    ...
    def hooked(hidden_states=None, encoder_hidden_states=None, timestep=None,
        ...
        with torch.no_grad():
            ...
            rec["temb"].append(transformer.time_guidance_embed(scaled, guidance).clone())
            if "cos" not in rec:
                ...
                rec["cos"] = torch.cat([txt_cos, img_cos], 0)[:, 0::2][None, :, None, :]
                rec["sin"] = torch.cat([txt_sin, img_sin], 0)[:, 0::2][None, :, None, :]
