...
def _permute_rows(linear, full_perm):
    """Clone a Linear with its output rows permuted (bias=False here)."""
...
class ExportKleinDouble(nn.Module):
    ...
    def __init__(self, block):
        ...
        # Q/K of BOTH streams get the same channel permutation (they are
        # concatenated and rotated together); V and the output projections do not.
        ...
        self.register_buffer("naq_w", attn.norm_added_q.weight.detach()[perm_t].clone())
        self.register_buffer("nak_w", attn.norm_added_k.weight.detach()[perm_t].clone())
        self.to_out = attn.to_out[0]
        self.to_add_out = attn.to_add_out
        self.ff_in, self.ff_out = block.ff.linear_in, block.ff.linear_out
        self.ffc_in, self.ffc_out = block.ff_context.linear_in, block.ff_context.linear_out
    ...
    def forward(self, hidden, encoder, cos, sin, mod_img, mod_txt):
        ...
        c_s_msa, c_sc_msa, c_g_msa, c_s_mlp, c_sc_mlp, c_g_mlp = mod_txt.chunk(6, dim=-1)
        ...
        encoder = encoder + c_g_mlp * self.ffc_out(self._swiglu(self.ffc_in(n_e)))
