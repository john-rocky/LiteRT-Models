...
transformer, once with the transformer swapped for the eight exported INTEGER-int8
chunks executed through the LiteRT `CompiledModel` API — and reports PSNR.

This is the gate that decided int8-over-int4 for Z-Image: random-input
correlations are the wrong metric for a diffusion model, the generated image is
the metric.
"""
import os
import sys
...
def tfl_run(name, *inputs):
    ...
    if name not in _CACHE:
        _CACHE[name] = CompiledModel.from_file(os.path.join(CHUNK_DIR, f"{name}.tflite"))
    model = _CACHE[name]
    ...
    for tensor_name, buffer, value in zip(sigs[key]["inputs"], in_buffers, inputs):
...
def dit_chunks(rm, hidden_states, encoder_hidden_states, timestep, img_ids, txt_ids):
    """The on-device DiT loop: host prep + eight int8 graphs, one resident at a time."""
    if img_ids.ndim == 3:
        img_ids = img_ids[0]
    if txt_ids.ndim == 3:
        txt_ids = txt_ids[0]
    ...
    hidden, encoder, mod_i, mod_t, mod_s = tfl_run(
        "kc_prep", hidden_states.numpy(), encoder_hidden_states.numpy(), temb.numpy())
    for i in range(2):
        hidden, encoder = tfl_run(f"kc_double{i}", hidden.numpy(), encoder.numpy(),
                                  cos.numpy(), sin.numpy(), mod_i.numpy(), mod_t.numpy())
...
def main():
    ...
    pipe = Flux2KleinPipeline.from_pretrained(REPO, torch_dtype=torch.float32).to("cpu")
    rm = pipe.transformer
    ...
    def hooked(hidden_states=None, encoder_hidden_states=None, timestep=None,
        ...
        with torch.no_grad():
            out = dit_chunks(rm, hidden_states, encoder_hidden_states, timestep,
                             img_ids, txt_ids)
        return Flux2Transformer2DModelOutput(sample=out) if return_dict else (out,)
