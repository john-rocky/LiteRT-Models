...
def latent_ids(grid):
    """(t, h, w, l) position ids for a grid x grid token map, like the pipeline."""
    t = torch.zeros(1)
...
def main():
    ...
    print(f"[load] {n/1e9:.3f} B params, {len(model.transformer_blocks)} double + "
          f"{len(model.single_transformer_blocks)} single")
