...
ADD applied to a BATCH_MATMUL result — `softmax(q @ k^T + mask[1,1,S,S])` returns
plausible-looking but wrong probabilities (device-only; the CPU reference and the
op checker are both clean). Materializing the mask across the head axis removes
the broadcast and the attention becomes bit-exact.
"""
import os
import sys
...
CHUNK = 9
TAPS = (9, 18, 27)
N_HEADS = 32          # the mask is materialized across this axis (see module docstring)
...
def main():
    ...
    torch.manual_seed(0)
    patch_repeat_kv()
    print("[patch] repeat_kv -> concat (ML Drift rejects both >4D and BROADCAST_TO)")
    ...
    # Reference must run the FULL stack: transformers applies the final RMSNorm to
    # hidden_states[num_layers], so truncating first would normalize the h27 tap.
    print(f"[load] {len(qwen.layers)} layers (taps {TAPS} are raw, below the final norm)")
    n = sum(p.numel() for p in qwen.layers[:N_LAYERS].parameters())
    print(f"[load] first {N_LAYERS} layers = {n/1e9:.2f} B params -> int8 ~{n/1e9:.2f} GB")

    inputs_embeds = torch.randn(1, SEQ, 2560) * 0.02
    position_ids = torch.arange(SEQ)[None]
    valid = 37                                   # pretend the prompt is 37 tokens
    causal = torch.full((SEQ, SEQ), -1e9).triu(1)
    pad = torch.zeros(SEQ)
    pad[valid:] = -1e9                           # padded keys are masked out
    mask = (causal + pad[None, :])[None, None]
    mask = mask.expand(1, N_HEADS, SEQ, SEQ).contiguous()   # [1,H,S,S], no broadcast
    ...
    for tap, layer_index in zip(taps, TAPS):
        ...
        corr = torch.corrcoef(torch.stack([want.flatten(), tap.flatten()]))[0, 1]
