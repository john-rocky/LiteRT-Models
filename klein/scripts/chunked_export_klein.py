...
class Prep(nn.Module):
    ...
    def __init__(self, dit):
        ...
        self.mod_img, self.mod_txt, self.mod_single = dit.mod_img, dit.mod_txt, dit.mod_single
    ...
    def forward(self, img_tokens, enc_hidden, temb):
        ...
        return (self.x_embedder(img_tokens), self.context_embedder(enc_hidden),
                self.mod_img(cond).unsqueeze(1), self.mod_txt(cond).unsqueeze(1),
...
class Final(nn.Module):
    ...
    def forward(self, joint, temb):
        ...
        hidden = safe_ln_noaffine(hidden) * (1 + scale)[:, None, :] + shift[:, None, :]
...
def main():
    ...
    for count in DOUBLE_SPLIT:
        doubles.append(DoubleChunk(list(dit.doubles[start:start + count])).eval())
        start += count
    ...
    with torch.no_grad():
        hidden, encoder, mod_i, mod_t, mod_s = prep(img_tokens, enc_hidden, temb)
    ...
    print(f"[compose] {1 + len(doubles) + len(singles) + 1} chunks sequential vs "
          f"monolithic: corr {corr:.6f}  max|diff| {(ref - out).abs().max():.2e}")

    if "--export" in sys.argv:
        q_export(prep, (img_tokens, enc_hidden, temb), "kc_prep")
        for i, chunk in enumerate(doubles):
            q_export(chunk, (hidden, encoder, cos, sin, mod_i, mod_t), f"kc_double{i}")
