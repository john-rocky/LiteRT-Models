...
Patches reused from Z-Image:
  * interleaved RoPE -> bake the even/odd de-interleave permutation into the Q/K
    ...
    RoPE becomes a contiguous half-split (SLICE) instead of a stride-2 GATHER_ND.
    Exact: q.k is invariant to a shared channel permutation.
...
def safe_rms_4d(x, weight, eps=1e-6):
    """safe_rms on [B,S,H,hd]: reshape to 3D first — an amax over a 4D tensor
    trips litert-torch's NHWC layout pass ("NHWC node rewriter not found: amax").
    The reduction is over the last dim either way, so this is exact.
    """
    b, s, h, d = x.shape
...
def safe_ln_noaffine(x, eps=1e-6):
    """fp16-safe LayerNorm without affine, max-normalized."""
    mu = x.mean(dim=-1, keepdim=True)
    d = x - mu
    m = d.abs().amax(dim=-1, keepdim=True).clamp_min(1e-4)
    ds = d / m
    return ds * torch.rsqrt(ds.pow(2).mean(dim=-1, keepdim=True) + (eps / (m * m)))


def rope_halfsplit(x, cos, sin):
    """Half-split real RoPE. x: [B,S,H,hd] pre-permuted to [evens; odds]."""
    x_e = x[..., :HALF]
    x_o = x[..., HALF:]
...
class ExportKleinSingle(nn.Module):
    ...
    def __init__(self, block):
        ...
        full = torch.tensor([h * HEAD_DIM + i for h in range(N_HEADS) for i in perm],
                            dtype=torch.long)
    ...
    def forward(self, hidden, cos, sin, shift, scale, gate):
        ...
        k = qkv[..., self.inner: 2 * self.inner].reshape(b, s, N_HEADS, HEAD_DIM)
        ...
        k = k.permute(0, 2, 1, 3)
        v = v.permute(0, 2, 1, 3)
        w_attn = torch.softmax(q @ k.transpose(-1, -2) * (HEAD_DIM ** -0.5), dim=-1)
...
def main():
    ...
    temb_mod = torch.cat([shift, scale, gate], dim=-1)   # split order: shift,scale,gate
    ...
    with torch.no_grad():
        ref = block(hidden, None, temb_mod, image_rotary_emb=(cos_full, sin_full))
    ...
    if "--convert" in sys.argv:
        ...
        litert_torch.convert(mine_mod, (hidden, cos, sin, shift, scale, gate)).export(out)
        import os
