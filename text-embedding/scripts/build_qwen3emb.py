...
# --model-dir: HF snapshot or your own merged fine-tune (save_pretrained() dir).
# Default reproduces the official ship: Qwen3-Embedding-0.6B downloaded next to this script.
...
class Qwen3EmbGPU(nn.Module):
    ...
    def forward(self, inputs_embeds):
        ...
        return safe_rms(h, self.final_norm_w)                               # [1,L,1024]
...
def main():
    ...
    ap.add_argument("--model-dir", default=MODEL_DIR,
                    help="HF snapshot or merged fine-tune directory")
    ap.add_argument("--out", default=os.path.join(HERE, "qwen3emb_gpu.tflite"))
    args = ap.parse_args()

    from transformers import AutoModel, AutoTokenizer
    print(f"loading {args.model_dir} ...", flush=True)
    base = AutoModel.from_pretrained(args.model_dir, torch_dtype=torch.float32).eval()
    ...
    with torch.no_grad():
        embeds = base.embed_tokens(ids)                                    # host does this
        ours = net(embeds)                                                 # [1,L,1024]
    ...
    rv = ref[0, real_len - 1]
    ov = ov / ov.norm(); rv = rv / rv.norm()
    corr = torch.dot(ov, rv).item()
    cos_full = F.cosine_similarity(ours.reshape(-1), ref.reshape(-1), dim=0).item()
    print(f"[parity] last-token emb cos={corr:.6f}  full-tensor cos={cos_full:.6f}", flush=True)
    if corr < 0.999:
        print("!! parity below 0.999 -- re-authoring differs from HF; fix before convert", flush=True)
