...
# (~4.1 GB) — important on near-full devices. Build the files with the scripts in this directory:
#
#   python scripts/build_dia2_temporal.py      # 30-layer temporal transformer, packed KV cache
#   python scripts/build_dia2_depformer.py     # 3 weight-scheduled depformer graphs
#   python scripts/build_dia2_mimi_decode.py   # one-shot 256-frame Mimi decode window
#   python scripts/export_dia2_assets.py       # baked fp16 embedding / projection tables
#   python scripts/bake_prefix.py              # voice prompt -> app/src/main/assets (13 kB)
#
# then:
