 ...
 *
 * Dia2's `parse_script` tokenizes each word standalone as
 * `tokenizer.encode(word, add_special_tokens=False)` — plain byte-level BPE. The [S1]/[S2]
 * speaker markers are handled by the caller (they map to reserved ids 49152/49153). The
 * pipeline:
...
class BpeTokenizer(ctx: Context) {
    ...
    /**
     * Tokenize `text` with byte-level BPE, verbatim — no trailing newline, no trimming.
     *
     * Dia2's `parse_script` encodes each word standalone as
     * `tokenizer.encode(word, add_special_tokens=False)`; a leading space is significant
     * (it becomes the GPT-2 `Ġ` marker), so callers must not strip it. Returns token ids.
     */
