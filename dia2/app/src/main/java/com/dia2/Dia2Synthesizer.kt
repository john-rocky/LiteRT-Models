...
class Dia2Synthesizer(context: Context) : Closeable {
    ...
    private fun sampleGuided(
        ...
        for (i in candidates) {
            if (cond[i] > maxLogit) maxLogit = cond[i]
        }
    ...
    private class Cache(val layers: Int, val kvHeads: Int, val maxSteps: Int) {
        ...
        fun mask() = FloatArray(maxSteps + 1) { MASK_NEG_INF }.also { m ->
            for (i in 0 until length) m[i] = 0f
            m[maxSteps] = 0f
        }
    ...
    /** One temporal step; returns (hidden, action logits, codebook-0 logits) and grows [cache]. */
    ...
    /** One depformer stage on weight set [weightId]; returns its hidden state. */
     ...
     * Building a prefix normally needs Whisper word timings and a Mimi *encoder*; both are done
    ...
    private inner class StateMachine(entries: List<Entry>) {
        ...
        /**
         * Advances one frame; returns the (main, second) tokens for the next model step.
         *
        ...
        fun process(step: Int, action: Int, isForced: Boolean = false): Pair<Int, Int> {
            ...
            if (token == NEW_WORD) {
                if (queue.isNotEmpty()) {
                    ...
                    if (entry.tokens.isNotEmpty()) {
                        for (id in entry.tokens) pending.addLast(id)
                        for (id in peek(SECOND_AHEAD)) lookahead.addLast(id)
