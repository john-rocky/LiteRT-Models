#!/usr/bin/env python3
"""Produce the shippable Matcha-TTS LiteRT artifacts (fp16) + verify parity.
...
sentences -> waveform corr. Run: ~/clipconv/bin/python convert_final.py [MAX_MEL]
"""
import _stub
import os, sys, json, math, numpy as np, torch, torch.nn as nn
...
ART = os.path.join(B.HERE, "artifacts"); os.makedirs(ART, exist_ok=True)
MAX_TEXT = 256
...
def main():
    ...
    print(f"MAX_TEXT={MAX_TEXT} MAX_MEL={MAX_MEL} mel_mean={mel_mean:.4f} mel_std={mel_std:.4f}")
    ...
    ex = emb_w[torch.zeros(1, MAX_TEXT, dtype=torch.long)]; tm0 = torch.ones(1, 1, MAX_TEXT)
    ...
    specs = [("textenc", te_r, (ex, tm0)), ("decoder", dec_r, (x0, mu0, te0, m0)),
             ("vocoder", gen_r, (mel0,))]
    fp16 = {}
    for name, mod, ins in specs:
        p32 = B.convert(mod, ins, os.path.join(ART, f"matcha_{name}.tflite"))
        p16 = B.to_fp16(p32, os.path.join(ART, f"matcha_{name}_fp16.tflite"))
        _, clean = B.opcheck(p16, name + "_fp16")
        s32, s16 = os.path.getsize(p32) / 1e6, os.path.getsize(p16) / 1e6
        print(f"  {name}: fp32 {s32:.1f}MB -> fp16 {s16:.1f}MB  GPU-clean={clean}")
        fp16[name] = p16

    from ai_edge_litert.interpreter import Interpreter
    it_te = Interpreter(model_path=fp16["textenc"]); it_te.allocate_tensors()
    it_dec = Interpreter(model_path=fp16["decoder"]); it_dec.allocate_tensors()
    it_gen = Interpreter(model_path=fp16["vocoder"]); it_gen.allocate_tensors()

    def tfl_te(emb_x, tmask):
        o = B.tfl_run(it_te, emb_x.numpy(), tmask.numpy())
        a, b = o; mu, lw = (a, b) if a.shape[1] == 80 else (b, a)
        return torch.from_numpy(mu.copy()), torch.from_numpy(lw.copy())
    def tfl_dec(x, mu, t_emb, mask):
        return torch.from_numpy(B.tfl_run(it_dec, x.numpy(), mu.numpy(), t_emb.numpy(), mask.numpy())[0].copy())
    def tfl_gen(mel):
    ...
    for s in SENTENCES:
        ...
        if ids.shape[-1] > MAX_TEXT:
            print(f"  SKIP (>{MAX_TEXT} tokens): {s!r}"); continue
        with torch.no_grad():
            wav_true, ylen, z = host_pipeline_masked(te_true, dec_true, gen_true, t_embed,
                emb_w, ids, mel_mean, mel_std, 10, MAX_TEXT, MAX_MEL)
        if ylen > MAX_MEL:
            print(f"  SKIP (>{MAX_MEL} frames, {ylen}): {s!r}"); continue
        wav_tfl, *_ = host_pipeline_masked(tfl_te, tfl_dec, tfl_gen, t_embed,
            emb_w, ids, mel_mean, mel_std, 10, MAX_TEXT, MAX_MEL, z=z)
        n = min(len(wav_true), len(wav_tfl))
        c = float(np.corrcoef(wav_true[:n], wav_tfl[:n])[0, 1])
        print(f"  corr={c:.5f}  frames={ylen}  {len(wav_tfl)/22050:.2f}s  {s!r}")
        try:
            import soundfile as sf
            sf.write(os.path.join(ART, f"sample_{SENTENCES.index(s)}.wav"), wav_tfl, 22050)
        except Exception:
            pass

    # export host-side tables + config (time-MLP now lives in the decoder graph)
    ...
    json.dump(cfg, open(os.path.join(ART, "config.json"), "w"), ensure_ascii=False, indent=2)
    print(f"\nartifacts in {ART}:")
