...
/**
 * Places365 scene recognition (ResNet18, CSAILVision) on the LiteRT CompiledModel GPU.
 *   image[1,3,224,224] (ImageNet-normalized) -> logits[1,365]   (365 scene categories)
 *
 * Pure CNN. Two numerically-exact GPU re-authorings: the global `AdaptiveAvgPool2d(1)` -> `mean(3).mean(2)`,
 * and the ResNet stem `MaxPool2d(3,s2,p1)` (which pads with -inf -> a `PADV2` the Mali delegate won't
 * delegate) -> zero-pad + valid max-pool (exactly equal since the pool follows a ReLU, inputs >= 0).
 * ~2 ms / 224x224 on a Pixel 8a; device output == PyTorch (corr 1.0, top-1 match).
 */
class PlacesClassifier(ctx: Context) : Closeable {

    companion object {
        const val SIZE = 224
        const val MODEL = "places_fp16.tflite"
    ...
    private val labels: List<String> = run {
        val txt = ctx.assets.open("places365_classes.json").bufferedReader().use { it.readText() }
    ...
    /** rgb: SIZE*SIZE*3 row-major [0,255]. Returns the top-k (scene label, softmax score). */
