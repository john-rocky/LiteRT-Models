# RTMPose-family universal converter -> GPU-clean TFLite for LiteRT CompiledModel.
# One script drives all five zoo modules (rtmpose / rtmface / rtmhand / rtmanimal / rtmw):
# pick the model with env vars (mmpose init_model underneath):
#   RTM_CFG=<path>   mmpose config: a path relative to mmpose/.mim/configs, or an
#                    existing local file (your own fine-tune config)
#   RTM_CKPT=<path>  checkpoint .pth: URL or local file (your own fine-tune)
#   RTM_H/RTM_W      input size (default 256x192; must match the config's input_size)
#   RTM_NAME=<stem>  output filename stem (default "rtm")
# Keypoint count flows from the head weights, so fine-tuned keypoint sets convert as-is.
# GPU patches (all numerically exact, weight-independent): ScaleNorm -> SafeRMSNorm
# (fp16 sum-of-squares overflow on Mali), GAU act@act BMM -> broadcast-reduce (Mali
# miscompute), RTMW PixelShuffle -> depth-to-space ZeroStuffConvT2d (6D lowering).
...
_cfg_env=os.environ["RTM_CFG"]
cfg=_cfg_env if os.path.exists(_cfg_env) else os.path.join(root, ".mim/configs", _cfg_env)
