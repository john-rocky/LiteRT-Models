...
/**
 * D-FINE-S object detection, fully on the LiteRT CompiledModel GPU (ML Drift / LITERT_CL).
 *
 * D-FINE (the SOTA real-time DETR — HGNetV2 backbone + hybrid AIFI/CCFM encoder + an FDR
 * Fine-grained-Distribution-Refinement decoder) split into two GPU graphs with a host step between:
 ...
 * lost. (This bug is why D-FINE first read as a "decoder fp16 wall" / 0 detections; the FDR decoder is
 * actually fine once it gets clean memory.) enc_output/enc_bbox_head are per-token, so
 * gather(f(x),idx) == f(gather(x,idx)); running them on the host over the 300 selected tokens is exact.
 *
 * Both graphs run 100% on the GPU on a Pixel 8a; device output matches PyTorch (real-image detections at
 * IoU 0.99-1.00). This is a still-image demo — the deformable decoder over the full 8400 tokens / 80×80
 * levels is GPU-compute-bound, not real-time on this device.
 */
class DFine(private val ctx: Context) : Closeable {

    companion object {
        const val SIZE = 640
        const val MODEL_A = "dfine_graphA_fp16.tflite"
        const val MODEL_B = "dfine_graphB_fp16.tflite"
