# Package the converted D-FINE-S graphs + host params into the Android module.
# Prereqs (run in ~/Downloads/meeting/dfine-work or any dir with the build scripts on PYTHONPATH):
#   python build_dfine_split.py fp16    # -> dfB_fp16.tflite  (Graph B, plain decoder)
#   python build_dfine_fix3.py  fp16    # -> dfA_fix3_fp16.tflite + dfine_host3_w.npz  (Graph A, the device fix)
# Then:
#   python pack_assets.py <work_dir> <module_dir>
# Copies the 2 tflites under the app's expected names and writes assets/host_params.bin + coco_labels.txt.
import sys, os, shutil
import numpy as np

work = sys.argv[1] if len(sys.argv) > 1 else "."
mod = sys.argv[2] if len(sys.argv) > 2 else os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
assets = os.path.join(mod, "android", "app/src/main/assets") if os.path.isdir(os.path.join(mod, "android")) \
    else os.path.join(mod, "app/src/main/assets")
os.makedirs(assets, exist_ok=True)

# Models go to filesDir via install_to_device.sh (NOT committed) — just rename them in the work dir.
shutil.copy(os.path.join(work, "dfA_fix3_fp16.tflite"), os.path.join(work, "dfine_graphA_fp16.tflite"))
shutil.copy(os.path.join(work, "dfB_fp16.tflite"), os.path.join(work, "dfine_graphB_fp16.tflite"))
print("renamed -> dfine_graphA_fp16.tflite, dfine_graphB_fp16.tflite  (push with install_to_device.sh)")

# host_params.bin — layout MUST match DFine.kt init{}.
z = np.load(os.path.join(work, "dfine_host3_w.npz"))
order = ["eo_w", "eo_b", "eo_g", "eo_beta", "bb0w", "bb0b", "bb1w", "bb1b", "bb2w", "bb2b", "valid", "anchors"]
blob = np.concatenate([z[k].astype(np.float32).ravel() for k in order])
blob.astype("<f4").tofile(os.path.join(assets, "host_params.bin"))
print(f"wrote host_params.bin ({blob.nbytes} bytes, {blob.size} floats)")

# Contiguous class labels from the model config (80 COCO for the official ship;
# a fine-tuned DF_MODEL_ID writes its own id2label here).
import build_dfine_split as S
net = S.build_net(); id2 = net.config.id2label
with open(os.path.join(assets, "coco_labels.txt"), "w") as f:
    f.write("\n".join(id2[i] for i in range(len(id2))) + "\n")
print(f"wrote coco_labels.txt ({len(id2)} classes)")
