import build_anime as B, numpy as np, torch, os
...
x=(np.asarray(im).astype(np.float32)/127.5-1.0).transpose(2,0,1)[None].copy()
m=B.build()
with torch.no_grad(): ref=m(torch.from_numpy(x)).numpy()[0]
np.save(f"{HERE}/an_ref.npy",ref); x.tofile(f"{HERE}/an_input.bin")
def save(arr,p): Image.fromarray((np.clip((arr.transpose(1,2,0)+1)/2,0,1)*255).astype(np.uint8)).save(p)
save(ref,f"{HERE}/an_torch.png"); im.save(f"{HERE}/an_content.png")
print(f"input {x.shape}; torch range [{ref.min():.2f},{ref.max():.2f}]")
from ai_edge_litert.interpreter import Interpreter
it=Interpreter(model_path=f"{HERE}/anime_{STYLE}_fp16.tflite"); it.allocate_tensors()
