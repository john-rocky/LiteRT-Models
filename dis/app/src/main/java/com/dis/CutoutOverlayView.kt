...
/**
 * Draws the cut-out composite over the camera preview, aligned to what PreviewView shows
 * (FILL_CENTER: frame scaled by max(view/frame), centered, overflow cropped).
 */
class CutoutOverlayView(context: Context) : View(context) {
    ...
    fun setComposite(b: Bitmap, sw: Int, sh: Int) { bmp = b; srcW = sw; srcH = sh; postInvalidateOnAnimation() }
